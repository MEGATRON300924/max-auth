...
const verifyOptionsBase: VerifyOptions = { issuer: env.JWT_ISSUER, audience: env.JWT_AUDIENCE };

export function signAccessToken(payload: Omit<AccessTokenPayload, "type">): string {
  return jwt.sign({ ...payload, type: "access" }, env.JWT_ACCESS_SECRET, { ...signOptionsBase, expiresIn: env.JWT_ACCESS_EXPIRES_IN } as SignOptions);
}

export function signRefreshToken(payload: Omit<RefreshTokenPayload, "type">): string {
  return jwt.sign({ ...payload, type: "refresh" }, env.JWT_REFRESH_SECRET, { ...signOptionsBase, expiresIn: env.JWT_REFRESH_EXPIRES_IN } as SignOptions);
}
...
export function decodeToken(token: string) { return jwt.decode(token); }
