...
export const oauthClientConfigService = {
  ...
  async upsert(ownerId: string, clientId: string, input: OAuthClientConfigInput) {
    ...
    const rows = await prisma.$queryRawUnsafe<any[]>(`INSERT INTO oauth_client_configs (client_id, application_type, authorized_origins, package_name, bundle_id, certificate_fingerprints, logo_url, display_name, website_url, manifest_url, verification_status, updated_at) VALUES ($1::uuid, $2, $3::text[], $4, $5, $6::text[], $7, $8, $9, $10, CASE WHEN $10 IS NULL THEN 'UNVERIFIED' ELSE 'PENDING' END, CURRENT_TIMESTAMP) ON CONFLICT (client_id) DO UPDATE SET application_type = EXCLUDED.application_type, authorized_origins = EXCLUDED.authorized_origins, package_name = EXCLUDED.package_name, bundle_id = EXCLUDED.bundle_id, certificate_fingerprints = EXCLUDED.certificate_fingerprints, logo_url = EXCLUDED.logo_url, display_name = EXCLUDED.display_name, website_url = EXCLUDED.website_url, manifest_url = EXCLUDED.manifest_url, verification_status = CASE WHEN EXCLUDED.manifest_url IS NULL THEN 'UNVERIFIED' ELSE 'PENDING' END, verified_at = CASE WHEN EXCLUDED.manifest_url IS NULL THEN NULL ELSE oauth_client_configs.verified_at END, updated_at = CURRENT_TIMESTAMP RETURNING application_type AS "applicationType", authorized_origins AS "authorizedOrigins", package_name AS "packageName", bundle_id AS "bundleId", certificate_fingerprints AS "certificateFingerprints", logo_url AS "logoUrl", display_name AS "displayName", website_url AS "websiteUrl", manifest_url AS "manifestUrl", verification_status AS "verificationStatus", verified_at AS "verifiedAt"`, dbId, input.applicationType, authorizedOrigins, input.packageName?.trim() || null, input.bundleId?.trim() || null, certificateFingerprints, logoUrl, displayName, websiteUrl, manifestUrl);
    return rows[0];
  },
