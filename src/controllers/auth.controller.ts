...
export const authController = {
  ...
  async google(req: Request, res: Response, next: NextFunction) { try { const ctx = getRequestContext(req); const { user, accessToken, refreshToken } = await googleAuthService.signIn(req.body.credential, ctx); setRefreshCookie(res, refreshToken, true); return ok(res, { user: sanitizeUser(user), accessToken }); } catch (err) { next(err); } },
  ...
  async resetPassword(req: Request, res: Response, next: NextFunction) { try { const ctx = getRequestContext(req); await authService.resetPassword(req.body.token, req.body.newPassword, ctx); return ok(res, { message: "Password reset successfully. Please log in again." }); } catch (err) { next(err); } },
  async changePassword(req: Request, res: Response, next: NextFunction) { try { const ctx = getRequestContext(req); await authService.changePassword(req.user!.sub, req.body.currentPassword, req.body.newPassword, ctx); return ok(res, { message: "Password changed successfully" }); } catch (err) { next(err); } },
  async deleteAccount(req: Request, res: Response, next: NextFunction) { try { const ctx = getRequestContext(req); await authService.deleteAccount(req.user!.sub, req.body.password, ctx); clearRefreshCookie(res); return ok(res, { message: "Account deleted" }); } catch (err) { next(err); } },
  async me(req: Request, res: Response, next: NextFunction) { try { const user = await userService.getProfile(req.user!.sub); return ok(res, { user: sanitizeUser(user) }); } catch (err) { next(err); } },
};
