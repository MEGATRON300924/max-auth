...
export const mfaController = {
  ...
  async enable(req: Request, res: Response, next: NextFunction) {
    try { noStore(res); const result = await mfaService.enable(req.user!.sub, req.body.password, req.body.code, getRequestContext(req)); return ok(res, result); } catch (err) { next(err); }
  },
  ...
  async disable(req: Request, res: Response, next: NextFunction) {
    try { noStore(res); const result = await mfaService.disable(req.user!.sub, req.body.password, req.body.code, getRequestContext(req)); return ok(res, result); } catch (err) { next(err); }
  },
};
